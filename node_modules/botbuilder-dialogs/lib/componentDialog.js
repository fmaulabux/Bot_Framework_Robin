...
class ComponentDialog extends dialog_1.Dialog {
    ...
    async returnAnswer(stepContext, qna, dialogInfo) {
        stepContext.context.activity.text = dialogInfo.retrieveDI()
        var qnaResults = await qna.qnaMaker.getAnswers(stepContext.context, qna.qnaMakerOptions);
        await stepContext.context.sendActivity(qnaResults[0].answer);
        dialogInfo.setDI("qnaQuery", stepContext.context.activity.text);
        dialogInfo.setDI("qnaResult", qnaResults[0].answer);
    }
    /**
